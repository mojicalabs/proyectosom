...
import com.bpd.hipos.vos.ItemVO;
 ...
 * @author u19730
 */
public class HIPOSBeneficiariosManager {

    Gson gson = new Gson();

    public BeneficiarioVO getBeneficiario(String codigoEmpleado) {
        System.out.println("getBeneficiario: " + codigoEmpleado);
        codigoEmpleado = codigoEmpleado.toUpperCase();
        BeneficiarioVO bVO = new BeneficiarioVO();
        if (codigoEmpleado != null || !codigoEmpleado.equals("0")) {
            ...
            if (data != null) {
                bVO.id = data.getId();
                bVO.codigoEmpleado = data.getCodigoEmpleado();
                bVO.sexoEmpleado = data.getSexoEmpleado();
                bVO.chkTitular = data.getChkTitular();

                ItemVO iVO1 = new ItemVO();
                iVO1.percent = data.getBonificacionPercent();
                iVO1.ammount = data.getBonificacionAmmount();
                bVO.bonificacion = iVO1;

                ItemVO iVO2 = new ItemVO();
                iVO2.percent = data.getBecasHijosPercent();
                iVO2.ammount = data.getBecasHijosAmmount();
                bVO.becasHijos = iVO2;

                ItemVO iVO3 = new ItemVO();
                iVO3.percent = data.getPlanRetiroPercent();
                iVO3.ammount = data.getPlanRetiroAmmount();
                bVO.planRetiro = iVO3;

                ItemVO iVO4 = new ItemVO();
                iVO4.percent = data.getSeguroMedicoInternacionalPercent();
                iVO4.ammount = data.getSeguroMedicoInternacionalAmmount();
                bVO.seguroMedicoInternacional = iVO4;

                bVO.dependientes = getDependientes(data.getCodigoEmpleado());
            } else {
                bVO.id = null;
                bVO.codigoEmpleado = "";
                bVO.sexoEmpleado = "";
                bVO.chkTitular = "false";
                ...
                bVO.dependientes = getDependientes(codigoEmpleado);
            }
            s.close();
        }
        return bVO;
        ...
        try {
            for (Iterator<HiposEmpleados> it = list.iterator(); it.hasNext();) {
                ...
                if (empVO != null) {
                    ...
                    iep.nombreEmpleado = empVO.nombreEmpleado;
                    iep.empPhoto = empVO.rutaPhoto + empVO.empPhoto;
                    iep.empPhoto = replaceSlashes(iep.empPhoto);
                    eVO.itemEmpPhoto = iep;
                }
                lout.add(eVO);
            }
    ...
    public String replaceSlashes(String data) {
    ...
    public List<EmpleadoVO> getEmpleadosByName(String empName, String empType) throws Exception {
        ...
        c.addOrder(Order.asc("codigoEmpleado"));
        List<HiposEmpleados> list = new ArrayList();
        List<EmpleadoVO> lout = new ArrayList();
        list = c.list();
        try {
            for (Iterator<HiposEmpleados> it = list.iterator(); it.hasNext();) {
                ...
                EmpleadoVO eVO = new EmpleadoVO();
                EmployeeVO empVO = getEmployee(he.getCodigoEmpleado());
                if (empVO.nombreEmpleado.contains(empName.toUpperCase())) {
                    eVO.id = he.getId();
                    eVO.codigoEmpleado = he.getCodigoEmpleado();
                    eVO.totalAmount = he.getTotalAmount();
                    eVO.minAmountPlanRetiro = he.getMinAmountPlanRetiro();
    ...
    public BeneficiarioVO setBeneficiario(BeneficiarioVO iaaVO) throws Exception {
        Session s = getMySession();

        System.out.println("setBeneficiario: " + gson.toJson(iaaVO));

        HiposBeneficiarios data = new HiposBeneficiarios();

        if (iaaVO.id != null && iaaVO.id != 0) {
            data.setId(iaaVO.id);
        }

        data.setCodigoEmpleado(iaaVO.codigoEmpleado.toUpperCase());
        data.setSexoEmpleado(iaaVO.sexoEmpleado.toUpperCase());
        data.setChkTitular(iaaVO.chkTitular);
        data.setBecasHijosPercent(iaaVO.becasHijos.percent);
        data.setBecasHijosAmmount(iaaVO.becasHijos.ammount);
        data.setBonificacionPercent(iaaVO.bonificacion.percent);
        data.setBonificacionAmmount(iaaVO.bonificacion.ammount);
        data.setPlanRetiroPercent(iaaVO.planRetiro.percent);
        data.setPlanRetiroAmmount(iaaVO.planRetiro.ammount);
        data.setSeguroMedicoInternacionalPercent(iaaVO.seguroMedicoInternacional.percent);
        data.setSeguroMedicoInternacionalAmmount(iaaVO.seguroMedicoInternacional.ammount);
        ...
        try {
            ...
            System.out.println("Error en delEmpleado: " + gson.toJson(data));
            System.out.println(e.getMessage());
            throw e;
        }
        ...
        List qObjInDb = getDependientes(iaaVO.codigoEmpleado);
        List evaDBStr = new ArrayList();
        List evaFrmStr = new ArrayList();
        for (Iterator<DependienteVO> it6 = qObjInDb.iterator(); it6.hasNext();) {
            DependienteVO evObj = it6.next();
            evaDBStr.add(evObj.id);
        }
        if (iaaVO.dependientes != null) {
            if (iaaVO.dependientes.size() > 0) {
                for (Iterator<DependienteVO> it7 = iaaVO.dependientes.iterator(); it7.hasNext();) {
                    DependienteVO elObj = it7.next();
                    if (elObj != null) {
                        if (elObj.id != null) {
        ...
        if (qObjDiff.size() > 0) {
            int delCounterQ = delDependiente(qObjDiff);
        }

        if (iaaVO.dependientes != null) {
            if (iaaVO.dependientes.size() >= 0) {
                for (Iterator it = iaaVO.dependientes.iterator(); it.hasNext();) {
                    DependienteVO object = (DependienteVO) it.next();
                    if (object != null) {
                        HiposDependientes hd = new HiposDependientes();

                        if (object.id != null && object.id != 0) {
                            hd.setId(object.id);
                        }

                        hd.setCodigoEmpleado(iaaVO.codigoEmpleado);
                        hd.setParentesco(object.parentesco);
                        hd.setEdad(object.edad);
                        hd.setPercent(object.percent);
                        hd.setAmmount(object.ammount);
                        ...
                        try {
                            ...
                            System.out.println("Error en setBeneficiario: " + gson.toJson(hd));
                            System.out.println(e.getMessage());
                            throw e;
                        }
        ...
        s.close();
        System.out.println("END BeneficiarioVO: " + gson.toJson(iaaVO));
        return iaaVO;
    }
